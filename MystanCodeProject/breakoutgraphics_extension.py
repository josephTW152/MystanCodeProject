...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH, paddle_height=PADDLE_HEIGHT,
        ...
        self.paddle.fill_color = 'DarkBlue'
        ...
        # Score
        self.score = GLabel('Score : ', x=0, y=20)
    ...
    def set_ball_velocity(self, add_x, add_y):
        if abs(self.ball.dx)+add_x < self.bricks.width:
            if self.ball.dx > 0:
                self.ball.dx += add_x
            else:
                self.ball.dx -= add_x
        if abs(self.ball.dy)+add_y < self.bricks.height:
            if self.ball.dy > 0:
                self.ball.dy += add_y
            else:
                self.ball.dy -= add_y
    ...
    def paddle_move(self, event):
        # The paddle can move between the middle and the bottom  of the window.
        ...
        elif event.y >= self.window.height - self.paddle.height:
            y = self.window.height - self.paddle.height
        else:
            y = event.y - self.paddle.height/2
        self.set_paddle_position(x, y)
